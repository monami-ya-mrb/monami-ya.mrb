...
parse_string(parser_state *p)
{
  ...
  while ((c = nextc(p)) != end || nest_level != 0) {
    if (hinf && (c == '\n' || c < 0)) {
      ...
      if (c < 0) {
        static const char msg1[] = "can't find heredoc delimiter \"";
  ...
  case '$':
    ...
    case '7': case '8': case '9':
      ...
      tokfix(p);
      {
        ...
        if (n > INT_MAX) {
#define ERROR_MSG(x) "capture group index must be <= " #x
          yyerror(p, ERROR_MSG(INT_MAX));
#undef ERROR_MSG
          return 0;
        }
...
{
  static const mrbc_context zero = { 0 };
  mrbc_context *c;
...
load_exec(mrb_state *mrb, parser_state *p, mrbc_context *c)
{
  ...
  if (!p->tree || p->nerr) {
    if (p->capture_errors) {
      ...
      mrb->exc = mrb_obj_ptr(mrb_exc_new_str(mrb, E_SYNTAX_ERROR, message));
      mrb_parser_free(p);
      return mrb_undef_value();
    }
    else {
